$(document).ready(function() {
	...
	$("span.folder, span.file").live("click", function() {
		...
		var nodeId=this.id;
		var nodeIdValue = $("#"+this.id).attr("value");
		var path = nodeIdValue.replace(new RegExp("△","gm"),"/");

		// 取得节点信息
		$.getJSON("./configAjax/getZNode", {path: path}, function(resultMap) {
		...
		// 取得子节点，展开文件夹
		$.getJSON("./configAjax/getChildren", {path: path}, function(resultMap) {
		var msg = resultMap.msg;
		...
		if (znodeList == null || znodeList.length == 0) {
		  ...
		  $("#addZNode").attr("disabled", "disabled");
		} else {
		  // 当文件夹前为加号时
		  if ($("#" + nodeId).prev().hasClass("collapsable-hitarea")) {
			...
			$("#znode_path").attr("disabled", "disabled");
			$("#addZNode").attr("disabled", "disabled");
			$("#deleteZNode").attr("disabled", "disabled");
			...
			$.each(znodeList, function(key, znode) {
			  var subNodeId=nodeId+"_"+index;
			  index++;
			  if (znode.stat.numChildren == 0) {
				  items.push('<li><span id="'+subNodeId+'" value="' + znode.id + '" class="file">' + znode.name + '('+znode.data.substring(0,20)+')</span></li>');
			  } else {
				  items.push('<li class="closed"><span id="'+subNodeId+'" value="' + znode.id + '" class="folder">' + 
					  znode.name + '('+znode.data.substring(0,20)+')</span><ul></ul></li>');
			  }
			});
			items.push("</ul>");
	...
	$("#deleteZNode").click(function() {
	  ...
	  $.get("./configAjax/deleteZNode", {path: path}, function(msg) {
		  if (msg == "succ") {
			  ...
			  if (parentPath == null || parentPath == "") {
				  var delNodeId = path.replace(new RegExp("/","gm"),"△");
				  $("#" + delNodeId).parent("li").remove();
			  } else {
				  var parentZnodeId = parentPath.replace(new RegExp("/","gm"),"△");
				  $("#" + parentZnodeId).click();
			  }
		  } else {
			  $("#errDiv").html(msg);
	...
	$("#addZNode").click(function() {
		var path = $("#znode_path").val();
		var data = $("#znode_data").val();
		$.post("./configAjax/addZNode", {path: path, data: data}, function(msg) {
			if (msg == "succ") {
	...
	$("#addZNodeRoot").click(function() {
	  ...
	  $("#znode_path").attr("disabled", "disabled");
	  $("#deleteZNode").attr("disabled", "disabled");
	  $("#updateZNode").attr("disabled", "disabled");
	...
	$("#addZNodeSub").click(function() {
	  ...
	  if (parentNode == null || parentNode == undefined || parentNode.length == 0) {
		  ...
		  setZNodeInfo(null);
		  $("#znode_name").val("temp");
		  var parentPath = parentNode.attr("value").replace(new RegExp("△","gm"),"/");
	 ...
	 * Nodex配置
	 */
	$("#nodexNode").click(function(){
		openwindow('./configAjax/goAddNodexConfig', "NodexWindow", 800, 400);
	})
	
	/**
	 * 导入
	 */
	$("#importNode").click(function() {
	  var parentNode = $("span[selected='selected']");
	  if (parentNode == null || parentNode == undefined || parentNode.length == 0) {
		  alert("请选择一个节点");
	  } else {
		  var parentPath = parentNode.attr("value").replace(new RegExp("△","gm"),"/");
		  openwindow("./configAjax/goUpload?path="+parentPath, "MsgWindow", 400, 200);
	  }
	});
	...
	function openwindow(url,name,iWidth,iHeight)
	 {
	  ...
	  var myWindow=window.open(url,name,'height='+iHeight+',,innerHeight='+iHeight+',width='+iWidth+',innerWidth='+iWidth+',top='+iTop+',left='+iLeft+',toolbar=no,menubar=no,scrollbars=auto,resizeable=no,location=no,status=no');
	...
	$("#exportNode").click(function() {
	  ...
	  if (parentNode == null || parentNode == undefined || parentNode.length == 0) {
		  ...
		  var parentPath = parentNode.attr("value").replace(new RegExp("△","gm"),"/");
		  if(confirm("确定要导出吗？")){
			  jQuery('<form action="./configAjax/exportNode" ><input type="text" name="path" value="'+parentPath+'"/></form>').appendTo('body').submit().remove();
		  }
	  }
	});
	...
	$("div.hitarea").live("click", function() {
		if ($(this).hasClass("collapsable-hitarea")) {
		  var nodeId = $(this).next().attr("value");
			var path = nodeId.replace(new RegExp("△","gm"),"/");
